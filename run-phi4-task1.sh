#!/usr/bin/env bash
# Run Task 1 (direct sentiment classification) for Microsoft Phi-4-reasoning-vision-15B
...
# GPU 1 is typically occupied by another process, so we use --4bit to fit in GPU 0 (~8 GB).
#
# Configurations:
#   - sigma3 (alpha3): p3, p5
#   - sigma5 (alpha5): p3, p5
#
# Results are saved to data/phi4-only/
...
SAVE_PATH="${SCRIPT_DIR}/data/phi4-only"
GPU_IDS="0,1"
