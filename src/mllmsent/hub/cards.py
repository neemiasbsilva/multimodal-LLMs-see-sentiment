...
Fine-tuned text classifiers from **"Multimodal LLMs See Sentiment"**. Each checkpoint
scores the sentiment of an image *description* produced by a multimodal LLM, which is
the second stage of the MLLMsent pipeline.
...
See {GITHUB_URL} for the citation entry.
...
Every input and every output of **"Multimodal LLMs See Sentiment"**: the image
descriptions generated by six multimodal LLMs, the sentiment labels derived from the
PerceptSent annotations, and the complete per-fold results of all 141 experiments.
